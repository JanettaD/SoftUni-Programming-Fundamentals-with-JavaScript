function storeProvision(stock, orderedProducts) {
    ...
    for (let i = 0; i < stock.length; i += 2) {
        ...
        //we convirt to a Number and take the next index of the stock arr
        let qty = Number(stock[i + 1]);
        //get into the products object(which is still empty), create a key which will be the name of the product and the value will be the qty;
    ...
    for (let j = 0; j < orderedProducts.length; j += 2) {
        ...
        if (productName in products) {
            //if yes- we get into the object with products, we find exactly this key and add the current quantyty; 
            products[productName] += qty;
